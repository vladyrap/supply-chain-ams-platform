...
export default function EscalationN2Page() {
  ...
  const canView = effective ? hasPermission(effective, "escalamiento_n2", "view", rbac.roles) : false;
  if (!canView) {
    return <AccessLockedCard screen="escalamiento_n2" reason="El Centro de Escalamiento Nivel 2 requiere al menos rol con permiso de visualización." />;
  }
  const canEdit      = effective ? hasPermission(effective, "escalamiento_n2", "edit", rbac.roles) : false;
  const canConfigure = effective ? hasPermission(effective, "escalamiento_n2", "configure", rbac.roles) : false;
  const canApprove   = effective ? hasPermission(effective, "escalamiento_n2", "approve", rbac.roles) : false;

  return (
    <EscalationCenter
      actingUserId={effective?.id || "anonymous"}
      canEdit={canEdit}
