...
export default function TestingIntelligencePage() {
  ...
  const canView = effective ? hasPermission(effective, "testing_intelligence", "view", rbac.roles) : false;
  if (!canView) {
    return <AccessLockedCard screen="testing_intelligence" reason="Testing Intelligence SAP requiere al menos rol con permiso de visualización." />;
  }
  const canEdit      = effective ? hasPermission(effective, "testing_intelligence", "edit", rbac.roles) : false;
  const canConfigure = effective ? hasPermission(effective, "testing_intelligence", "configure", rbac.roles) : false;

  return (
    <TestingIntelligenceCenter
      actingUserId={effective?.email || effective?.id || "anonymous"}
      canEdit={canEdit}
